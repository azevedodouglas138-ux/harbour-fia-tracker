...
document.querySelectorAll('.bbg-fn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    if (btn.dataset.tab === 'tab-apresentacao') loadApresentacaoTab();
  });
});
...
document.getElementById('cfg-save')?.addEventListener('click', async () => {
  ...
  if (res.ok) {
    ...
    _apresLoaded = false; // force reload next time APRES. tab is opened
...
let _apresSlide     = 0;
const _APRES_N      = 6;
let _apresChartPerf = null;
let _apresChartDist = null;
let _apresLoaded    = false;
let _apresRawData   = {};

function apresGo(delta)  { apresGoTo(_apresSlide + delta); }

function apresGoTo(idx) {
  const prev = _apresSlide;
  _apresSlide = Math.max(0, Math.min(idx, _APRES_N - 1));
  document.getElementById(`apres-slide-${prev}`).style.display    = 'none';
  document.getElementById(`apres-slide-${_apresSlide}`).style.display = '';
  document.querySelectorAll('.apres-dot').forEach((d, i) =>
    d.classList.toggle('active', i === _apresSlide));
  document.getElementById('apres-counter').textContent = `${_apresSlide + 1} / ${_APRES_N}`;
}

document.addEventListener('keydown', e => {
  if (!document.getElementById('tab-apresentacao')?.classList.contains('active')) return;
  if (e.key === 'ArrowRight') apresGo(1);
  else if (e.key === 'ArrowLeft') apresGo(-1);
});

async function loadApresentacaoTab() {
  if (_apresLoaded) return;
  _apresLoaded = true;

  try {
    const [config, portfolio, perfChart, perfInd, annualData,
           varData, captureData, teData, scData, concData, liqData, distData] = await Promise.all([
      fetch('/api/fund-config').then(r => r.json()),
      fetch('/api/portfolio').then(r => r.json()),
      fetch('/api/performance-chart').then(r => r.json()),
      fetch('/api/performance-indicators').then(r => r.json()),
      fetch('/api/annual-returns').then(r => r.json()),
      fetch('/api/risk/var?window=252').then(r => r.json()).catch(() => ({})),
      fetch('/api/risk/capture?window=252').then(r => r.json()).catch(() => ({})),
      fetch('/api/risk/tracking-error?window=252').then(r => r.json()).catch(() => ({})),
      fetch('/api/risk/sortino-calmar').then(r => r.json()).catch(() => ({})),
      fetch('/api/risk/concentration').then(r => r.json()).catch(() => ({})),
      fetch('/api/risk/liquidity').then(r => r.json()).catch(() => ({})),
      fetch('/api/risk/return-distribution?window=252').then(r => r.json()).catch(() => ({})),
    ]);

    _apresRawData = { config, portfolio, perfChart, perfInd, annualData,
                      varData, captureData, teData, scData, concData, liqData, distData };

    apresRenderSlide0(config, portfolio, annualData);
    apresRenderSlide1(perfChart, perfInd);
    apresRenderSlide2(annualData);
    apresRenderSlide3(perfInd, varData, captureData, teData, scData, portfolio);
    apresRenderSlide4(portfolio, concData, liqData);
    apresRenderSlide5(distData);

    apresGoTo(0);
  } catch (err) {
    console.error('[apres] load error:', err);
    _apresLoaded = false;
  }
}

// ── Slide 0: Sobre o Fundo ───────────────────────────────────────
function apresRenderSlide0(config, portfolio, annualData) {
  const quota    = portfolio.quota || {};
  const cota     = quota.cota_estimada ?? config.quota_fechamento;
  const numCotas = quota.num_cotas ?? config.num_cotas;
  const aum      = cota && numCotas ? cota * numCotas : null;

  const refDate = config.data_fechamento || '';
  const refEl   = document.getElementById('apres-ref-date');
  if (refEl && refDate) refEl.textContent = `Ref.: ${refDate}`;

  const aumEl = document.getElementById('apres-aum');
  if (aumEl) aumEl.textContent = aum ? `R$ ${(aum / 1e6).toFixed(2)}M` : '—';

  const quotaEl = document.getElementById('apres-quota');
  if (quotaEl) quotaEl.textContent = cota ? cota.toFixed(8) : '—';

  const inception = annualData.inception_date;
  const incEl = document.getElementById('apres-inception');
  if (incEl && inception) {
    const [y, m, d] = inception.split('-');
    incEl.textContent = `${d}/${m}/${y}`;
  }

  const descEl = document.getElementById('apres-descricao');
  if (descEl) {
    descEl.textContent = config.descricao_fundo ||
      'Descrição do fundo não configurada. Acesse CONFIGURAÇÕES para adicionar.';
    descEl.style.color = config.descricao_fundo ? '#ccc' : '#666';
  }
}

// ── Slide 1: Performance Acumulada ──────────────────────────────
function apresRenderSlide1(perfChart, perfInd) {
  const series     = perfChart.series     || [];
  const benchmarks = perfChart.benchmarks || {};
  if (!series.length) return;

  const baseFund = series[0]?.fund ?? 1;
  const baseIbov = series.find(s => s.ibov)?.ibov;
  const cdiMap   = benchmarks.cdi || {};
  const cdiKeys  = Object.keys(cdiMap).sort();
  const baseCdi  = cdiKeys.length ? cdiMap[cdiKeys[0]] : null;

  const labels   = series.map(s => s.date);
  const fundData = series.map(s => s.fund  ? +((s.fund  / baseFund - 1) * 100).toFixed(2) : null);
  const ibovData = series.map(s => s.ibov && baseIbov ? +((s.ibov / baseIbov - 1) * 100).toFixed(2) : null);
  const cdiData  = baseCdi ? labels.map(d => cdiMap[d] ? +((cdiMap[d] / baseCdi - 1) * 100).toFixed(2) : null) : null;

  const canvas = document.getElementById('apres-perf-canvas');
  if (!canvas) return;
  if (_apresChartPerf) { _apresChartPerf.destroy(); _apresChartPerf = null; }

  const datasets = [
    { label: 'HARBOUR', data: fundData, borderColor: '#ff8c00', borderWidth: 2,   fill: false, pointRadius: 0, tension: 0.1 },
    { label: 'IBOV',    data: ibovData, borderColor: '#4488ff', borderWidth: 1.5, fill: false, pointRadius: 0, tension: 0.1 },
  ];
  if (cdiData) datasets.push(
    { label: 'CDI', data: cdiData, borderColor: '#00cc44', borderWidth: 1.5, fill: false, pointRadius: 0, tension: 0.1, borderDash: [5, 3] }
  );

  _apresChartPerf = new Chart(canvas, {
    type: 'line',
    data: { labels, datasets },
    options: {
  ...
  const ind    = perfInd.data || {};
  const fmtPct = (v, d=2) => v != null ? `${v >= 0 ? '+' : ''}${v.toFixed(d)}%` : '—';
  const clr    = v => v == null ? '' : v >= 0 ? 'pos' : 'neg';
  const setM   = (id, val, fmt='pct') => {
    const el = document.getElementById(id); if (!el) return;
    el.textContent = fmt === 'pct' ? fmtPct(val) : val != null ? val.toFixed(2) : '—';
    if (fmt === 'pct') el.className = `apres-metric-val ${clr(val)}`;
  };
  setM('am-total-ret', ind.total?.ret);
  setM('am-ytd-ret',   ind.no_ano?.ret);
  setM('am-12m-ret',   ind['12m']?.ret);
  setM('am-sharpe',    ind.total?.sharpe, 'num');

  const volEl = document.getElementById('am-vol');
  if (volEl) volEl.textContent = ind.total?.vol != null ? ind.total.vol.toFixed(2) + '%' : '—';
}

// ── Slide 2: Rentabilidade Anual ────────────────────────────────
function apresRenderSlide2(annualData) {
  const years = annualData.years || [];
  const tbody = document.getElementById('apres-annual-tbody');
  const tfoot = document.getElementById('apres-annual-tfoot');
  if (!tbody) return;
  ...
  tbody.innerHTML = years.map(yr => `
    <tr>
      <td>${yr.year}</td>
      <td class="num ${cl(yr.fund_year)}">${fmtPct(yr.fund_year)}</td>
      <td class="num ${cl(yr.ibov_year)}">${fmtPct(yr.ibov_year)}</td>
      <td class="num ${cl(yr.cdi_year)}">${fmtPct(yr.cdi_year)}</td>
      <td class="num ${cl(yr.alpha)}">${fmtPct(yr.alpha)}</td>
    </tr>`).join('');

  if (years.length) {
    const last = years[years.length - 1];
    tfoot.innerHTML = `<tr class="apres-accum-row">
      <td>ACUMULADO</td>
      <td class="num ${cl(last.fund_accum)}">${fmtPct(last.fund_accum)}</td>
      <td class="num ${cl(last.ibov_accum)}">${fmtPct(last.ibov_accum)}</td>
      <td class="num ${cl(last.cdi_accum)}">${fmtPct(last.cdi_accum)}</td>
      <td class="num">—</td>
    </tr>`;
  }
}
...
// ── Slide 3: Risco ───────────────────────────────────────────────
function apresRenderSlide3(perfInd, varData, captureData, teData, scData, portfolio) {
  const ind = perfInd.data || {};
  const sc  = scData.windows || {};

  const set = (id, val, fmt='pct') => {
    const el = document.getElementById(id); if (!el) return;
    if (val == null) { el.textContent = '—'; el.className = 'apres-risk-val'; return; }
    el.textContent = fmt === 'pct' ? `${val >= 0 ? '+' : ''}${val.toFixed(2)}%` : val.toFixed(2);
    el.className = 'apres-risk-val ' + (fmt === 'pct' ? (val >= 0 ? 'pos' : 'neg') : '');
  };

  set('ar-sharpe', ind.total?.sharpe,              'num');
  set('ar-vol',    ind.total?.vol,                  'pct');
  set('ar-dd',     sc.total?.max_dd,                'pct');
  set('ar-var',    varData.var_95_1d_pct,           'pct');
  set('ar-beta',   portfolio.weighted_beta,          'num');
  set('ar-te',     teData.tracking_error,            'pct');
  set('ar-ir',     teData.information_ratio,         'num');
  set('ar-cup',    captureData.upside_capture,       'num');
  set('ar-cdn',    captureData.downside_capture,     'num');
}

// ── Slide 4: Concentração e Liquidez ────────────────────────────
function apresRenderSlide4(portfolio, concData, liqData) {
  const rows = [...(portfolio.rows || [])]
    .filter(r => r.pct_total)
    .sort((a, b) => (b.pct_total || 0) - (a.pct_total || 0))
    .slice(0, 5);

  const top5El = document.getElementById('apres-top5');
  if (top5El) {
    top5El.innerHTML = rows.map(r => `
      <div class="apres-pos-row">
        <span class="apres-pos-ticker">${r.ticker}</span>
        <div class="apres-pos-bar-wrap">
          <div class="apres-pos-bar" style="width:${Math.min(100, r.pct_total)}%"></div>
        </div>
        <span class="apres-pos-pct">${r.pct_total.toFixed(1)}%</span>
      </div>`).join('');
  }

  const hhiEl = document.getElementById('apres-hhi-row');
  if (hhiEl && concData.hhi != null) {
    hhiEl.innerHTML = `<span class="apres-hhi-label">HHI</span>
      <span class="apres-hhi-val">${concData.hhi} <small>(${concData.hhi_label || ''})</small></span>
      <span class="apres-hhi-top5">Top 5: ${concData.top5_pct != null ? concData.top5_pct.toFixed(1) + '%' : '—'}</span>`;
  }

  const liqEl = document.getElementById('apres-liq-items');
  if (liqEl) {
    const items = [
      ['1 DIA',   liqData.portfolio_liq_1d_pct],
      ['5 DIAS',  liqData.portfolio_liq_5d_pct],
      ['10 DIAS', liqData.portfolio_liq_10d_pct],
    ];
    liqEl.innerHTML = items.map(([lbl, val]) => `
      <div class="apres-liq-item">
        <div class="apres-liq-label">${lbl}</div>
        <div class="apres-liq-val">${val != null ? val.toFixed(1) + '%' : '—'}</div>
        ${val != null ? `<div class="apres-liq-bar-wrap"><div class="apres-liq-bar" style="width:${Math.min(100, val)}%"></div></div>` : ''}
      </div>`).join('');
  }
}

// ── Slide 5: Distribuição de Retornos ───────────────────────────
function apresRenderSlide5(distData) {
  const canvas = document.getElementById('apres-dist-canvas');
  if (!canvas) return;
  if (_apresChartDist) { _apresChartDist.destroy(); _apresChartDist = null; }

  const labels     = distData.bin_centers || [];
  const counts     = distData.counts      || [];
  const ibovCounts = distData.ibov_counts;

  const datasets = [{
    label: 'HARBOUR',
    data:  counts,
    backgroundColor: labels.map(v => v >= 0 ? 'rgba(255,140,0,0.75)' : 'rgba(255,51,51,0.6)'),
    borderWidth: 0,
  }];
  if (ibovCounts) datasets.push({
    label: 'IBOV',
    data:  ibovCounts,
  ...
  _apresChartDist = new Chart(canvas, {
    type: 'bar',
    ...
    options: {
      ...
      scales: {
        x: { ticks: { color: '#888', maxRotation: 45, maxTicksLimit: 10 }, grid: { color: '#222' } },
  ...
  const s = (id, txt, cls) => {
    const el = document.getElementById(id); if (!el) return;
    el.textContent = txt;
    if (cls !== undefined) el.className = `apres-metric-val ${cls}`;
  };
  s('ad-skew',  distData.skewness    != null ? distData.skewness.toFixed(3)  : '—');
  s('ad-kurt',  distData.kurtosis    != null ? distData.kurtosis.toFixed(3)  : '—');
  s('ad-pos',   distData.pct_positive != null ? distData.pct_positive + '%'  : '—');
  s('ad-best',  fmtPct(distData.best_day),  cl(distData.best_day));
  s('ad-worst', fmtPct(distData.worst_day), cl(distData.worst_day));
}

// ── Export: PDF ──────────────────────────────────────────────────
async function apresExportPDF() {
  const btn = document.getElementById('apres-btn-pdf');
  ...
  try {
    ...
    for (let i = 0; i < _APRES_N; i++) {
      const el = document.getElementById(`apres-slide-${i}`);
      const prev = el.style.display;
      el.style.display = '';
      const cvs = await html2canvas(el, { scale: 1.5, useCORS: true, backgroundColor: '#141414' });
      el.style.display = prev;
      if (i > 0) pdf.addPage();
      pdf.addImage(cvs.toDataURL('image/jpeg', 0.92), 'JPEG', 0, 0, pdfW, pdfH);
    }

    const ts = new Date().toISOString().slice(0, 10).replace(/-/g, '');
    pdf.save(`harbour_fia_${ts}.pdf`);
  } catch (err) {
    console.error('[apres] PDF error:', err);
...
// ── Export: PPTX ─────────────────────────────────────────────────
async function apresExportPPTX() {
  const btn = document.getElementById('apres-btn-ppt');
  if (btn) { btn.disabled = true; btn.textContent = 'GERANDO...'; }
  try {
    const d       = _apresRawData;
    const config  = d.config    || {};
    const pf      = d.portfolio || {};
    const ind     = d.perfInd?.data || {};
    const quota   = pf.quota   || {};
    const numCotas = quota.num_cotas ?? config.num_cotas;
    const cota    = quota.cota_estimada ?? config.quota_fechamento;
    const aum     = cota && numCotas ? `R$ ${(cota * numCotas / 1e6).toFixed(2)}M` : '—';
    const sc      = d.scData?.windows || {};
    const today   = new Date().toLocaleDateString('pt-BR');

    const inception = d.annualData?.inception_date;
    const inceptionFmt = inception ? inception.split('-').reverse().join('/') : '—';

    const top5 = [...(pf.rows || [])].filter(r => r.pct_total)
      .sort((a, b) => (b.pct_total || 0) - (a.pct_total || 0))
      .slice(0, 5)
      .map(r => ({ ticker: r.ticker, pct_total: r.pct_total }));

    const perfCanvas = document.getElementById('apres-perf-canvas');
    const distCanvas = document.getElementById('apres-dist-canvas');

    const payload = {
      fund_name: 'HARBOUR IAT FIF AÇÕES RL',
      ref_date:  today,
      descricao: config.descricao_fundo || '',
      stats: {
        aum, quota: cota ? cota.toFixed(8) : '—',
        inception: inceptionFmt, benchmark: 'IBOV', strategy: 'Long-Only Ações',
      },
      indicators: { total: ind.total || {}, no_ano: ind.no_ano || {}, '12m': ind['12m'] || {} },
      annual_years: d.annualData?.years || [],
      risk: {
        sharpe:          ind.total?.sharpe,
        vol:             ind.total?.vol,
        max_dd:          sc.total?.max_dd,
        var_95:          d.varData?.var_95_1d_pct,
        beta:            pf.weighted_beta,
        tracking_error:  d.teData?.tracking_error,
        info_ratio:      d.teData?.information_ratio,
        upside_capture:  d.captureData?.upside_capture,
        downside_capture: d.captureData?.downside_capture,
      },
      concentration: {
        hhi: d.concData?.hhi, hhi_label: d.concData?.hhi_label,
        top5_pct: d.concData?.top5_pct, top5,
      },
      liquidity: {
        liq_1d_pct:  d.liqData?.portfolio_liq_1d_pct,
        liq_5d_pct:  d.liqData?.portfolio_liq_5d_pct,
        liq_10d_pct: d.liqData?.portfolio_liq_10d_pct,
      },
      dist: {
        skewness: d.distData?.skewness, kurtosis:     d.distData?.kurtosis,
        pct_positive: d.distData?.pct_positive,
        best_day: d.distData?.best_day, worst_day: d.distData?.worst_day,
      },
      images: {
        perf_chart: perfCanvas ? perfCanvas.toDataURL('image/png') : null,
        dist_chart: distCanvas ? distCanvas.toDataURL('image/png') : null,
      },
    };
    ...
    a.href = url;
    a.download = `harbour_fia_${new Date().toISOString().slice(0,10).replace(/-/g,'')}.pptx`;
    a.click();
    URL.revokeObjectURL(url);
  } catch (err) {
    console.error('[apres] PPTX error:', err);
    alert('Erro ao gerar PPT: ' + err.message);
  } finally {
